...
import {LuCopy, LuGlobe, LuUsers, LuX } from 'react-icons/lu'
...
const Share = ({flowId}) => {
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if(!sharePanelRef.current.contains(e.target)){
        setSharePanel(false)
      }
    }
  ...
  return (
    <div className='relative' ref={sharePanelRef}>
      <button onClick={() => setSharePanel(true)} className='bg-blue-500 text-white rounded px-2 py-1.5 font-medium  text-smb flex items-center gap-1 hover:bg-blue-600 active:bg-blue-700'>
        <LuUsers /> Share
      </button>
      {sharePanel && 
